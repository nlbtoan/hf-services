...
class ProductGatlingTest extends Simulation {
    ...
    val authorization_header = "Basic " + Base64.getEncoder.encodeToString("hfProfileServiceapp:bXktc2VjcmV0LXRva2VuLXRvLWNoYW5nZS1pbi1wcm9kdWN0aW9uLWFuZC10by1rZWVwLWluLWEtc2VjdXJlLXBsYWNl".getBytes(StandardCharsets.UTF_8))
    ...
    val scn = scenario("Test the Product entity")
        ...
        .formParam("client_id", "hfProfileServiceapp")
        ...
        .repeat(2) {
            ...
            .get("/hfprofileservice/api/products")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new product")
            .post("/hfprofileservice/api/products")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "productId":null, "name":"SAMPLE_TEXT", "weight":null}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_product_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created product")
                .get("/hfprofileservice${new_product_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created product")
            .delete("/hfprofileservice${new_product_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
